...
| id | select_type        | table    | type   | possible_keys | key     | key_len | ref                       | rows | Extra                                        |
+----+--------------------+----------+--------+---------------+---------+---------+---------------------------+------+----------------------------------------------+
|  1 | PRIMARY            | region   | ref    | PRIMARY,name  | name    | 25      | const                     |    1 | Using where; Using temporary; Using filesort |
|  1 | PRIMARY            | part     | ref    | PRIMARY,size  | size    | 4       | const                     | 3800 | Using where                                  |
|  1 | PRIMARY            | partsupp | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.part.P_PARTKEY       |    1 | Using where                                  |
|  1 | PRIMARY            | supplier | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.partsupp.PS_SUPPKEY  |    1 |                                              |
|  1 | PRIMARY            | nation   | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.supplier.S_NATIONKEY |    1 | Using where                                  |
|  2 | DEPENDENT SUBQUERY | region   | ref    | PRIMARY,name  | name    | 25      | const                     |    1 | Using where                                  |
|  2 | DEPENDENT SUBQUERY | partsupp | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.part.P_PARTKEY       |    1 |                                              |
|  2 | DEPENDENT SUBQUERY | supplier | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.partsupp.PS_SUPPKEY  |    1 |                                              |
|  2 | DEPENDENT SUBQUERY | nation   | eq_ref | PRIMARY       | PRIMARY | 4       | tpch.supplier.S_NATIONKEY |    1 | Using where                                  |
+----+--------------------+----------+--------+---------------+---------+---------+---------------------------+------+----------------------------------------------+
162 rows in set (0.61 sec)
